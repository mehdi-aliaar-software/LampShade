...
function makePicName(source, dist) {

    let fileName01 = '~/assets/images/'+ source.value.substring(12, source.value.length);

    alert(source.value);

    //$('#' + dist).val(convertToPicName(value));
    //$('#' + dist).val();

    //const value = $('#' + source).val();
    //$('#' + dist).val(fileName01);

    dist.value = fileName01;
}
